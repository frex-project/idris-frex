...
FinPowerSetoidAlgebra n a =
  ...
  let lemma : (j : Fin $ arity op) ->
      ...
      lemma j = CalcWith (cast a) $
        ...
        ~~ index j (map (index i)  yss) .=<(indexNaturality _ _ _)
  in CalcWith (cast a) $
  ...
  ~~ index i (map (a.Sem op)
                  (transpose yss))..<(indexHomomorphismLemma n a _ _ _)
...
representation =
  ...
  { Iso = MkIsomorphism
      ...
      , Bwd = MkSetoidHomomorphism
               ...
               $ \phis, psis, prf, i => CalcWith (cast a) $
                 ...
                 ~~ psis.H i ...(prf i)
                 ~~ index i (tabulate psis.H) .=<(indexTabulate _ _)
      , Iso = IsIsomorphism
  ...
  , FwdHomo = \op, xss, i => CalcWith (cast a) $
      ...
      ~~ a.Sem op (map (index i)                  xss )...((eval i).preserves op xss)
      ~~ a.Sem op (map (\xs => (fwd xs).H i)      xss ).=.(Refl)
      ~~ a.Sem op (map (\phi => phi.H i) (map fwd xss)).=<(cong (a.Sem op) $ mapFusion _ _ _)
  }
